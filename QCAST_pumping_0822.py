...
filename1 = '/home/ustc-test1/quantum/Q-Network/positions1.txt'#1780行
filename2 = '/home/ustc-test1/quantum/Q-Network/positions2.txt'#1268行
...
#        print(len(graph.keys()), graph)
        ...
        for i in range(4):                    #degree of nodes in lattice network <= 4
            path = find_best_path(self,self.src.ID,self.dst.ID,graph,k,self.bandwidth,self.fidelity)
            if path:
                print(path)
...
def find_best_path(request,src:int,dst:int,graph:dict,k:list,bandwith,fidelity_target):
    T_path = [graph[src]['T']] * len(graph)
    C_path = [graph[src]['C']] * len(graph)
    unvisited = list(graph.keys())
