...
filename1 = 'positions1.txt'
filename2 = 'positions2.txt'
...
class Link:
    ...
    def back_up(self):
        a_list = []  # 存储每轮备份操作次数的列表
        prob_backup_list = []  # 每轮引入备份操作后的纯化成功概率
        threshold_list = []  # 存储每轮阈值的列表
        threshold_multi_final = 0  # 判断结束标志的临时变量

        threshold_list.insert(0, self.threshold_all)
        for i in range(self.r - 1):
            threshold_list.insert(0, 0)

        for i in range(self.r):
            a_list.append(1)

        for i in range(self.r):
            prob_backup_list.append(0)

        for i in range(self.r + 1):
            self.n_backup_list.append(1)

        while threshold_multi_final < self.threshold_all:
            i_r = 0
            if self.r == 0:
                break
            while i_r < self.r:
                if self.probability_list[-(1 + i_r)] < threshold_list[-(1 + i_r)]:
                    if self.n_backup_list[-(1 + i_r)] == 1:  # 该情况其实只针对最后一轮时，目标量子比特数量为1时，无需运用cdf计算
                        prob_backup_list[-(1 + i_r)] = 1 - (1 - self.probability_list[- (1 + i_r)]) ** a_list[-(1 + i_r)]
                    else:  # 针对非最后一轮时，运用cdf计算当尝试a次，成功超过n（目标量子比特数量）的概率超过95% 
                        # 震撼了，居然在这里也卡住过
                        prob_backup_list[-(1 + i_r)] = 1 - binom.cdf(self.n_backup_list[-(1 + i_r)] - 1, a_list[-(1 + i_r)],
                                                                     self.probability_list[-(1 + i_r)])
                    # 当运算所得的概率小于阈值时，逐步增加a值，以获得最小满足条件的a
                    if prob_backup_list[-(1 + i_r)] < threshold_list[-(1 + i_r)]:
                        a_list[-(1 + i_r)] += 1
                        continue
                    # 当概率大于所设阈值时，我们将所计算得出的概率更新为该轮阈值，以给其他部分的阈值设置预留余量
                    else:
                        threshold_list[-(1 + i_r)] = prob_backup_list[-(1 + i_r)]
                else:
                    prob_backup_list[-(1 + i_r)] = self.probability_list[-(1 + i_r)]
                    if i_r != 0:
                        a_list[-(1 + i_r)] = self.n_backup_list[-i_r]
                        threshold_list[-(1 + i_r)] = prob_backup_list[-(1 + i_r)]
                    else:
                        a_list[-(1 + i_r)] = 1
                        threshold_list[-(1 + i_r)] = prob_backup_list[-(1 + i_r)]
                # 判断是否满足退出循环的条件
                threshold_multi_final = 1
                for i in range(self.r):
                    threshold_multi_final *= threshold_list[i]
                # 更新阈值列表，更新方法为：总阈值/已计算出的轮数的阈值之积
                threshold_multi = 1
                for i in range(i_r + 1):
                    threshold_multi *= threshold_list[-(1 + i)]
                if i_r <= self.r - 2:
                    threshold_list[-(i_r + 2)] = self.threshold_all / threshold_multi
                if 0 < i_r <= self.r - 2 and threshold_list[-(i_r + 2)] > 0.995:
                    a_list[-(1 + i_r)] += 1
                    threshold_list[-(i_r + 2)] = 0
                    continue
                if i_r <= self.r - 1:
                    self.n_backup_list[-(i_r + 2)] = a_list[-(1 + i_r)] * 2
                if i_r <= self.r - 2:
                    a_list[-(2 + i_r)] = self.n_backup_list[-(i_r + 2)]
                i_r += 1
...
class Request:
    ...
    def path_finder(self, nodes: list, edges: list, k: list):
        ...
        # graph = {node.ID:{'C':len(node.qmemory.mem_positions),'T':node.T, 'Link_count': 0} for node in nodes}
...
#               print("end to end fidelity", self.fidelity, "target", link.decomposition_fidelity, "original", link.initial_fidelity)
    ...
    def resource_allocation(self):
        ...
        if optimize(self.path_set, path, self.bandwidth) == False:
            self.success = -1
            return False
        else: 
            print(self.id)
...
def find_best_path(request,src:int,dst:int,graph:dict,k:list):
    ...
    metric[src - 1] = k[0] * T_path[src - 1] / C_path[src - 1]
    path[src - 1] =[src]
    while(unvisited):
...
#        print(current_node_id)
        ...
        for neighbor_node_id in current_neighbors:
            ...
            new_metric = metric[current_node_id - 1] + k[0] * (temp_T / temp_C - T_path[current_node_id - 1] / C_path[current_node_id - 1]) - \
...
#         Pnm_nm = Decimal('0')
#         #swapping_prob_decimal = Decimal(str(self.swapping_probability))
#         for bm in range(nm, min(path_set[m], key=lambda x: x.allocatedcapacity).allocatedcapacity+1):
#             Pnm_nm += Decimal(math.comb(bm,nm))  * Decimal(str(self.swapping_probability)) ** (len(path_set[m]) * nm) * (Decimal(str(1 - self.swapping_probability)) ** len(path_set[m])) ** (bm - nm) * Decimal(str(self.Pbm[bm]))
# #        print("Cmn(bm,nm)" , Pnm_nm)
#         self.Pnm.append(Pnm_nm)
    ...
    def backtrack(m, path_indices, product):
        ...
        for nm in range(len(path[m].Pnm)):
            ...
            if sum(path_indices) + sum1 >= qubit_num:
                ...
                # backtrack(m + 1, path_indices, Decimal(product * path[m].Pnm[nm]))
                path_indices.pop()
#        print(len(path[m].Pnm))

    res = []
    backtrack(0, [], 1)
    # backtrack(0, [], Decimal('1'))
#    print(res)
#    time1 = time.time()
    # print("pu :",sum(res))
    return sum(res)
...
#        print("Pu" , Pu)
        if(Pu >= threshold):        
            if((-np.log10(1-Pu))>5):
                for i in range(len(path_set)):
                    # print("Pu" , Pu)
                    res=1 + int(min(path_set[i], key=lambda x: x.allocatedcapacity).allocatedcapacity * 0.1)
                    # res=1 + int(min(path_set[i], key=lambda x: x.allocatedcapacity).allocatedcapacity * (-np.log10(1-Pu)-4)/(-np.log10(1-Pu) + 50))
                    for j in range(len(path_set[i])):
                        path_set[i][j].allocatedcapacity -=res # 这里减的太快了？
            ...
            #     return False
            # 不加下面这句就没事
            # max_row, max_col = max(((i, j) for i, row in enumerate(path_set) for j, x in enumerate(row)), key=lambda x: (path_set[x[0]][x[1]].allocatedcapacity, path_set[x[0]][x[1]].n_backup_list[0])) 
    ...
    for i in range(len(path_set)):
        for j in range(len(path_set[i])):
            path_set[i][j].allocatedcapacity += 3
...
class DEJMPS_Distillation(ServiceProtocol):
    ...
    def handle_request_2(self, request, start_time = None, **kwargs):
        ...
        """ with open(filename1, 'a') as file1:
            file1.write(f'Node {self.node.ID} Distillation, role={self.role}, path_id={request.path_id}, receive distillation request\n') """
    ...
    def run(self):
        ...
        while True:
            ...
            while len(self.queue)>0:
                ...
                if len(request.positions)<request.bit_num*math.pow(2, len(request.backup_amount)-1):
                ...
                yield from self.distillation(request)
    ...
    for i in range(scale):
        for j in range(scale - 1):
            ...
            if configured:
                ...
                fidelity = random.gauss(0.95, 0.03)
                # print(fidelity)
                if fidelity > 1:
                    fidelity = 1
