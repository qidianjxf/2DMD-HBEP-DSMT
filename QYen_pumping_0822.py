...
class Link:
    ...
    def prior_knowledge(self):
        ...
        while fidelity < float(self.decomposition_fidelity):
...
#        print(len(graph.keys()), graph)
        ...
        for i in range(4):                    #degree of nodes in lattice network <= 4
            path = find_best_path(self,self.src.ID,self.dst.ID,graph,k,self.bandwidth)
            if path:
                print(path)
...
#               print("end to end fidelity", self.fidelity, "target", link.decomposition_fidelity, "original", link.initial_fidelity)
                link.run()
                # write_to_file(str(link.left_node), filename2)
                # write_to_file(str(link.right_node), filename2)
                # write_to_file(str(link.n_backup_list[0]), filename2)
                # print(link.max_capacity,link.n_backup_list[0],link.rescapacity,link.allocatedcapacity)

            # print("\n")
                # if link.rescapacity < bottleneck:
                #     bottleneck = link.rescapacity
            # for link in path:                   # avoid too large combination num 
            #     if link.rescapacity > bottleneck * 2:
            #         link.rescapacity = 2 * bottleneck
            #         link.allocatedcapacity = link.rescapacity
    def resource_allocation(self):
        routing_req.append(self.id)
        for i in range(len(self.path_set)):
            for j in range(len(self.path_set[i])):
                self.path_set[i][j].allocatedcapacity = min(math.ceil(self.bandwidth*1.3), self.path_set[i][j].allocatedcapacity) 
                #self.path_set[i][j].allocatedcapacity = 5 #因Netsquid的限制，此参数不能大于7
                #self.path_set[i][j].allocatedcapacity = 5
...
def check_entanglement(req_list, node_req, bit_id):
    ...
    if req.achieved_bandwidth >= req.bandwidth:# and req.achieved_bandwidth < 5:
...
def yen_algorithm(graph, src, dst, K_paths):
    """ Yen算法生成K条最短路径 """
    def dijkstra_shortest_path(graph, src, dst, excluded_nodes=set(), excluded_edges=set()):
        # Dijkstra实现
        heap = [(0, src, [])]
        visited = set()
        while heap:
            ...
            if u == dst:
                return (cost, path)
            if u in excluded_nodes:
                continue
            for v in graph[u]:
                if isinstance(v, int) and (u, v) not in excluded_edges:
                    heapq.heappush(heap, (cost + 1, v, path)) 
        return (float('inf'), [])

    # 生成第一条最短路径
    A = []
    B = []
    cost, path = dijkstra_shortest_path(graph, src, dst)
    if path:
        heapq.heappush(A, (cost, path))
    
    # 生成后续路径
    for k in range(1, K_paths):
        if not A:
            break
        cost, spur_path = heapq.heappop(A)
        yield spur_path  # 返回当前最短路径
        
        # 偏离操作生成新候选路径
        for i in range(len(spur_path)-1):
            spur_node = spur_path[i]
            root_path = spur_path[:i+1]
            
            # 禁用已选路径的边
            excluded_edges = set()
            if i < len(spur_path)-1:
                excluded_edges.add((spur_node, spur_path[i+1]))
            
            # 生成偏离路径
            cost_spur, spur_subpath = dijkstra_shortest_path(
                graph, spur_node, dst, 
                excluded_nodes=set(root_path[:-1]),
                excluded_edges=excluded_edges
            )
            if spur_subpath:
                new_path = root_path[:-1] + spur_subpath
                new_cost = len(new_path) - 1  # 跳数作为成本
                if new_path not in [p for (c, p) in A + B]:
                    heapq.heappush(B, (new_cost, new_path))
        
        # 将B中的路径合并到A
        while B:
            item = heapq.heappop(B)
            heapq.heappush(A, item)

def find_best_path(request, src, dst, graph, k, M):
    # 使用Yen算法生成M²条最短路径
    K = M ** 2
    all_paths = []
    try:
        # 生成M²条路径
        generator = yen_algorithm(graph, src, dst, K)
        for path in generator:
            if len(path) > 0 and path not in all_paths:
    ...
    if final_paths:
        ...
        T_path_val = max(graph[node]['T'] for node in best_path)
        C_path_val = min(graph[node]['C'] for node in best_path)
        F_path = sum(graph[best_path[i]][best_path[i+1]][2] 
                   for i in range(len(best_path)-1))
        D_path = sum(graph[best_path[i]][best_path[i+1]][3]/100000 
                   for i in range(len(best_path)-1))
        
        
        request.req_num = getattr(request, 'req_num', 0)
        routing_T[request.req_num - 1].append(T_path_val)
        routing_C[request.req_num - 1].append(C_path_val)
        routing_F[request.req_num - 1].append(F_path/(len(best_path)-1))
...
class DEJMPS_Distillation(ServiceProtocol):
    ...
    def __init__(self, node, name = None, direction = '', role = True):
        super().__init__(node, name)
        self.register_request(self.request, self.distillation)
    ...
    def prepared_entangle_batch(self, path_id, success_indices, fail_indices, local_positions, remote_positions):
        """批量生成新的纠缠对并存入指定内存位置"""
        new_entanglements = []
        ori_f = 0.85  # 初始保真度（需与主函数一致）
        
        # 处理成功案例：每个成功索引生成一个纠缠对
        for i in success_indices:
            # 创建纠缠对并应用噪声
            qubits = ns.qubits.create_qubits(2)
            self.node_statistics[self.node]["bit_consumed"] += 2
            ns.qubits.assign_qstate(qubits, ns.b00)
            theta = noise_parameter(ori_f, 0)
            noise_sim = ops.create_rotation_op(theta)
            qapi.operate(qubits[1], noise_sim)
            # 记录要存储的位置（远程位置为邻居节点自身内存）
            new_entanglements.append((
                qubits[0],
                remote_positions[2*i],  # 邻居节点自身位置
                qubits[1],              # 调用方位置（需通过信号传递）
            ))
        
        # 处理失败案例：每个失败索引生成两个纠缠对
        for i in fail_indices:
            qubits = ns.qubits.create_qubits(4)
            self.node_statistics[self.node]["bit_consumed"] += 4
            # 第一个纠缠对
            ns.qubits.assign_qstate(qubits[0:2], ns.b00)
            theta = noise_parameter(ori_f, 0)
            noise_sim = ops.create_rotation_op(theta)
            qapi.operate(qubits[1], noise_sim)
            # 第二个纠缠对
            ns.qubits.assign_qstate(qubits[2:4], ns.b00)
            theta = noise_parameter(ori_f, 0)
            noise_sim = ops.create_rotation_op(theta)
            qapi.operate(qubits[3], noise_sim)
            # 记录两个纠缠对的位置
            new_entanglements.extend([
                (qubits[0], remote_positions[2*i], qubits[1]),
                (qubits[2], remote_positions[2*i+1], qubits[3])
            ])
        
        # 将生成的量子比特存入邻居节点内存
        for q_nei, pos_nei, q_caller in new_entanglements:
            while self.node.qmemory.busy:
                yield self.await_program(self.node.qmemory)
            self.node.qmemory.put(q_nei, pos_nei)
            # 通过信号将调用方的量子比特信息传递回去
            self.send_signal(
                signal=self._entangle_qubit_ready_signal,
                result=(q_caller, local_positions[new_entanglements.index((q_nei, pos_nei, q_caller))//2])
            )
        
        # 发送批量完成信号
        self.send_signal(self._entangle_complete_signal)
    ...
    def run(self):
        ...
        while True:
            ...
            while len(self.queue) > 0:
                ...
                yield from self.pumping(request)

    def pumping(self, request):
        #match和非match的对不上
        l = request.path_id #(request.path_id用于区分不同的路径)
        round_num = len(request.backup_amount)-1 #request.backup_amount备份的量子比特的数量
        # print("node:", self.node, self.direction, "r_num", round_num)
        positions = request.positions#（当前节点用于处理请求的量子内存位置列表)
        """ for i in range(len(request_path_id)):
            if l in request_path_id[i]:
                distillation_R[i].append(len(positions))
                distillation_T[i].append(round_num) """
        ...
        self.ava_positions = ava_positions#(当前的量子内存位置)
        self.make_entangle_pair_token=True#（标记是否由当前的节点负责生成新的纠缠对，True为生成)
        r_aim = len(request.backup_amount) - 1 #(纯化的轮数)

        self.round_count=[0]*len(request.positions) #(记录每个位置参与的纯化轮数)
        ...
        ori_f = 0.85 #(初始纠缠对的保真度（默认0.8）)
       # pos = self.neighbour.measure_pos
        if len(ava_positions) < request.bit_num * 2: #(检查资源是否足够,request.bit_num为纠缠对的数量)
            raise Exception(f'{self.name}: Provided bit number is not enough to handle the request.')
        # qa, = self.node.qmemory.peek(ava_positions[0])#(从当前可用节点中查看？第一个位置的量子比特)
        if request.path_id in self.neighbour.ready_list:#（检查当前邻居节点是否存在当前请求的path_id）
            # qb, = self.neighbour.node.qmemory.peek(self.neighbour.ready_list[request.path_id][0])#(从邻居的ready_list中查看邻居节点的第一个位置的量子比特)
            # ori_f = ns.qubits.fidelity([qa, qb], ns.b00, squared=True)#(计算纠缠对保真度)
            #print(f'{self.name}: initial fidelity={ori_f}')
            self.send_signal(self._prepared_signal)#(发送信号？通知谁？通知邻居本节点已经准备好了吗)
        else:
            self.ready_list[request.path_id] = request.positions#如果没有准备好，将请求的request.positions存入自己节点的ready_list
            yield self.await_signal(self.neighbour, self._prepared_signal)#异步等待邻居的信号，直到邻居完成资源的准备工作
        r_cur = []#记录每个纠缠对当前完成的纯化轮数
        for i in range(int(len(request.positions) / 2)):#每个纠缠对占用两个位置，所以循环次数为request.positions/2
            r_cur.append(0)#初始化r_cur为0
        self.succ_positions = []#存储成功通过一轮纯化的量子比特的位置
        self.fail_positions = []#存储失败后被释放的量子比特位置
        results = []#存储每队纠缠对的测量结果？
        if request.path_id in self.neighbour.ready_list:#二次检查，用来确保双方的节点完全同步?
        ...
        counter=0 #限制循环次数?
        while (min(r_cur) < r_aim and counter<2):#所有纠缠对的纯化轮数均达标，或者循环已经执行2次
            i = 0
            results = []
            res_nei=[]

            self.node_statistics[self.node]["pur_round"] += 1

            while i < len(request.positions):
                if r_cur[int(i / 2)] >= r_aim:#若当前纠缠对的纯化轮数已经达标，记录成功的位置并跳过本次循环
                    self.succ_positions.append(request.positions[i + 1])
                    i += 2
                    results.append(2) #存储本节点测量的结果？？？？什么意思
                    continue
                while self.node.qmemory.busy:#检查当前节点的量子内存是否忙碌
                    yield self.await_program(self.node.qmemory)#等待内存空闲
                yield self.node.qmemory.execute_program(self.program, qubit_mapping=[ava_positions[i], ava_positions[i + 1]])#在量子内存的指定位置（ava_positions[i] 和 ava_positions[i+1]）执行预定义的量子程序 self.program，联合测量和经典通信逻辑？？？
                with open(filename1, 'a') as file1:
                    file1.write(f'Node {self.node.ID} Distillation, role={self.role}, path_id={request.path_id}, executed DEJMPS program, round={r_cur}\n')
                try:
                    m, = self.program.output["m"]#从量子程序的输出中获取测量结果，m表示测量结果，0/1 完成，-1 协议执行失败
                except KeyError:
                    m = -2
                results.append(m)
                i += 2
            self.refresh_key()#更新密钥？
            # print("node", self.node, self.direction,"key counter",self.key_counter)
            if self.key_counter == self.neighbour.key_counter:#确保处于同一轮次
                # self.send_signal(self._round_sync_signal)
                self.neighbour.get_result([results, ava_positions])#获取邻居的结果
                res_nei = self.neighbour.measure_results#获取的什么结果???测量结果以及位置？
            ...
            if len(results) != len(res_nei):#结果长度不一致，抛出异常
                raise Exception(f'{self.name}: Measure results length is different from the neighbour.')
            if len(results) != int(len(request.positions) / 2):
                print(f'Warning: given {int(len(request.positions) / 2)} positions, but only get {len(results)} results.')
            prepared_entangle_positions = []#记录已通过_prep_ent_signal信号确认的纠缠对索引，避免重复生成：
            success_indices = []
            fail_indices = []

            # 第一轮：仅收集成功/失败的索引
            for i in range(len(results)):
                if results[i] == res_nei[i] and results[i] != -2 and results[i] != 2 and r_cur[i]<r_aim:
            ...
            for idx in success_indices:
                # 释放旧量子比特
                self.node.qmemory.pop(ava_positions[2 * idx])
                # 生成新纠缠对
                if self.neighbour.make_entangle_pair_token:
                    ...
                    # 生成并分配新纠缠对
                    qubits = ns.qubits.create_qubits(2)
                    self.node_statistics[self.node]["bit_consumed"] += 1
                    ns.qubits.assign_qstate(qubits, ns.b00)
                    theta = noise_parameter(ori_f, 0)
                    noise_sim = ops.create_rotation_op(theta)
                    qapi.operate(qubits[1], noise_sim)
                    # 分配量子比特到位置
                    self.node.qmemory.put(qubits[0], positions=ava_positions[2 * idx])
                    self.neighbour.node.qmemory.put(qubits[1], positions=self.neighbour.ava_positions[2 * idx])
                    self.neighbour.send_signal(self.neighbour._entangle_complete_signal)
                else:
                    self.neighbour.prepared_entangle(idx)
            ...
            for idx in fail_indices:
                # 释放旧量子比特
                self.node.qmemory.pop(ava_positions[2 * idx])
                self.node.qmemory.pop(ava_positions[2 * idx + 1])
                # 生成两对纠缠对（仅由token持有者执行）
                if self.neighbour.make_entangle_pair_token:
                    self.make_entangle_pair_token = False
                    yield self.await_signal(self, self._prep_ent_signal)
                    qubits = ns.qubits.create_qubits(4)
                    ...
                    # 分配两对纠缠对
                    for k in range(2):
                        self.node.qmemory.put(qubits[2*k], positions=ava_positions[2*idx +k])
                        self.neighbour.node.qmemory.put(qubits[2*k+1], positions=self.neighbour.ava_positions[2*idx +k])
                    self.neighbour.send_signal(self.neighbour._entangle_complete_signal)
                else:
                    self.neighbour.prepared_entangle(idx)
                    yield self.await_signal(self, self._entangle_complete_signal)
        ...
        req = get_req_by_pathid(self.node.running_list, l)#通过路径ID l 从节点的运行列表 running_list 中获取对应的请求对象 。running_list：存储当前节点正在处理的所有请求？？
        #print('self.ava_positions',self.ava_positions)
        #把ava_positions中元素为奇数位取出
        i = 0
        while i < 2*int(len(self.ava_positions)/2)-1:#循环遍历纠缠对的位置
            if r_cur[int(i/2)] >= r_aim:#判断纯化轮数是否达到目标轮数
                self.succ_positions.append(self.ava_positions[i+1])
            i += 2
        """ file2 = open(filename2, 'a')
        file2.write(f'Node {self.node.ID} Distillation, role={self.role}, path_id={request.path_id}, succeeded positions={self.succ_positions}\n')
        file2.close() """
        if (req.prev_qubit_num == 0):#源节点？？
            self.node.src_protocol.get_positions(self.node.src_protocol.position(l, self.succ_positions))
        elif (req.next_qubit_num == 0):#目标节点？
            self.node.dst_protocol.get_positions(self.node.dst_protocol.position(l, self.succ_positions))
        else:
            self.node.swap_protocol.get_positions(self.node.swap_protocol.position(l, self.succ_positions))
        if l in self.ready_list:#如果当前的路径ID l 在 ready_list 中，则从 ready_list 中删除该路径ID
        ...
        #print("纯化消耗以及轮数:",self.node_statistics[self.node])
...
#            print(self.node.ID, self.queue)
            while len(self.queue) > 0:
                ...
                # write_to_file(str(request.path_id), filename2)
                # write_to_file(str(request.the_other_node), filename2)
                # write_to_file(str(request.positions), filename2)
    ...
    def run(self):
        while True:
            ...
            #    print("node", self.node.ID, "waiting_list_length", len(self.node.waiting_queue), "running_list_length", len(self.node.running_list))
                ...
                # write_to_file(str(current_request.path_id), filename2)
                # write_to_file(str(current_request.prev_node), filename2)
                # write_to_file(str(current_request.prev_qubit_num), filename2)
                # write_to_file(str(current_request.next_node), filename2)
                # write_to_file(str(current_request.next_qubit_num), filename2)
            ...
            # split a node request into two parts 
                ...
                if current_request.prev_qubit_num:
                    ...
                    # write_to_file(str(req_prev.path_id), filename1)
                    # write_to_file(str(req_prev.the_other_node), filename1)
                    # write_to_file(str(req_prev.positions), filename1)
                if current_request.next_qubit_num:
                    ...
                    # write_to_file(str(req_next.path_id), filename1)
                    # write_to_file(str(req_next.the_other_node), filename1)
                    # write_to_file(str(req_next.positions), filename1)
...
#print("distillation_R_success:",distillation_R_success)
...
new_dis_r=[x for x in distillation_R_average if x != 0]
new_dis_t=[x for x in distillation_T_average if x != 0]
...
print(f'distillation_R min:{min(new_dis_r)},max:{max(new_dis_r)},average:{sum(new_dis_r)/len(new_dis_r)}')
print(f'distillation_T min:{min(new_dis_t)},max:{max(new_dis_t)},average:{sum(new_dis_t)/len(new_dis_t)}')
